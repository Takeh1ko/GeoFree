## GeoFree — простой способ добавить блокирующиеся домены в hosts

GeoFree — это набор скриптов для Windows, Linux и macOS, которые удобно и безопасно добавляют записи в файл hosts, чтобы принудительно резолвить определённые домены на заданный IP-адрес. Метод не новый, репозиторий создан исключительно для удобства: один клик/команда и сервисы работа (записи добавлены без дубликатов, с резервной копией и возможностью отката).
...
После внесения изменений перезапустите приложения, при необходимости сбросьте DNS-кэш (см. раздел «Проверка и сброс DNS» ниже).
...
### Сброс DNS-кэша
- Windows (администратор, PowerShell/CMD):
```bat
ipconfig /flushdns
```

- Linux (в зависимости от демона):
```bash
sudo systemd-resolve --flush-caches || sudo resolvectl flush-caches || sudo service nscd restart
```
